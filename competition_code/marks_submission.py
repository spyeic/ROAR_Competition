...
class RoarCompetitionSolution:
    def __init__(
        ...
        self.lat_pid_controller = LatPIDController(config=get_lateral_pid_config())
    ...
    async def step(
        ...
        for i, section_ind in enumerate(self.section_indeces):
            if section_ind - 2 <= self.current_waypoint_idx <= section_ind + 2 \
    ...
    def get_lookahead_index(self, speed):
        num_waypoints = get_lookahead_value(speed)
    ...
    def average_point(self, current_speed):
        ...
        lookahead_value = get_lookahead_value(current_speed)
...
def find_k_values(current_speed: float, config: dict) -> np.array:
    k_p, k_d, k_i = 1, 0, 0
...
class LatPIDController:
    ...
    def run(self, vehicle_location, vehicle_rotation, current_speed, next_waypoint) -> float:
        ...
        k_p, k_d, k_i = find_k_values(current_speed=current_speed, config=self.config)
...
class ThrottleController:
    ...
    def get_throttle_and_brake(self, current_location, current_speed, current_section, waypoints):
        ...
        speed_data = [self.speed_for_turn(close_distance, target_speed1, current_speed),
    ...
    def speed_data_to_throttle_and_brake(self, speed_data: SpeedData):
        ...
        percent_speed_change = (speed_data.current_speed - self.previous_speed) / (
                self.previous_speed + 0.0001)  # avoid division by zero
        ...
        if percent_of_max > 1:
            ...
            if percent_of_max > 1 + (brake_threshold_multiplier * percent_change_per_tick):
                ...
                if self.brake_ticks <= 0 and not self.speed_dropping_fast(percent_change_per_tick,
                    ...
                    self.dprint(
                        "tb: tick" + str(self.tick_counter) + " brake: initiate counter" + str(self.brake_ticks))
                    return -1, 1
                else:
                    # speed is already dropping fast, ok to throttle because the effect of throttle is delayed
                    self.dprint("tb: tick" + str(self.tick_counter) + " brake: throttle early1: sp_ch=" + str(
                ...
                if self.speed_dropping_fast(percent_change_per_tick, speed_data.current_speed):
                    # speed is already dropping fast, ok to throttle because the effect of throttle is delayed
                    self.dprint("tb: tick" + str(self.tick_counter) + " brake: throttle early2: sp_ch=" + str(
                        percent_speed_change))
                    self.brake_ticks = 0  # done slowing down. clear brake_ticks
                    return 1, 0
                ...
                if percent_of_max > 1.02 or percent_speed_change > (-percent_change_per_tick / 2):
                    self.dprint("tb: tick" + str(self.tick_counter) + " brake: throttle down: sp_ch=" + str(
                        percent_speed_change))
            ...
            if self.speed_dropping_fast(percent_change_per_tick, speed_data.current_speed):
                ...
                self.dprint("tb: tick" + str(self.tick_counter) + " throttle: full speed drop: sp_ch=" + str(
                    percent_speed_change))
                return 1, 0
            ...
            if percent_of_max < 0.98 or percent_speed_change < -0.01:
                ...
                self.dprint(
                    "tb: tick" + str(self.tick_counter) + " throttle maintain: sp_ch=" + str(percent_speed_change))
                return throttle_to_maintain, 0
    ...
    # find speed_data with the smallest recommended speed
    ...
    def get_target_speed(self, radius: float, current_section):
        ...
        return max(20, min(self.max_speed, target_speed))  # clamp between 20 and max_speed
